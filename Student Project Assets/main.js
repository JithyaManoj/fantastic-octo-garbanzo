...
function preload(){
    dodle_modell=ml5.imageClassifier("DoodleNet");
}
function classifycanvas(){
    dodle_modell.classify(canvas,gotresult);
 }
...
function check_sketch() {
    timer_counter++;
    document.getElementById("time_sketch").innerHTML="time:"+ timer_counter;
...
function draw() {
    ...
    if (drawn_sketch == sketch) {
        answer_holder = "set";
        score++;
        document.getElementById("score_sketch").innerHTML="score :"+ score;
    }
}
function gotresult(e,r){
    if(e){
        ...
        confidence=confidence=Math.round(r[0].confidence*100);
